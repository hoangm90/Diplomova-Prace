...
import json
import get_data, update_color, random_sorting, color_to_time
...
def create_subjects_dictionary(lesson_raw):
    ...
    for lesson in lesson_raw:
        if lesson.get("subject_id") == "":
            ...
            if subjects.get(lesson["subject_id"]) == None:
                subjects[lesson["subject_id"]] = {}

            current_subject = subjects[lesson["subject_id"]]
...
def update_colors_to_database():
    ...
    for lesson_id in lessons:
        ...
        print(lesson_id + ": " + color_time[new_color])
        lesson_color_class.append({"lesson_id": lesson_id, "color": str(new_color), "chosen_classroom": chosen_cl, "time": color_time[new_color]})
    print('The smallest number of needed colors after testing 100 times is', smallest_number_color)

    # f = open("results.txt", "a")
    # jsstring = json.dumps(lesson_color_class)
    # f.write("\n\nNEW EXPERIMENT: \n\n")
    # f.write(jsstring)
    # f.close()
